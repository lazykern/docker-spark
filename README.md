...
* Spark 3.3.2 for Hadoop 3.3 with OpenJDK 8 and Scala 2.12
...
services:
  spark-master:
    image: phusitsom/spark-master:3.3.2-hadoop3
  ...
  spark-worker-1:
    image: phusitsom/spark-worker:3.3.2-hadoop3
  ...
  spark-history-server:
      image: phusitsom/spark-history-server:3.3.2-hadoop3
...
To start a Spark master:
    ...
    docker run --name spark-master -h spark-master -d phusitsom/spark-master:3.3.2-hadoop3

### Spark Worker
To start a Spark worker:

    docker run --name spark-worker-1 --link spark-master:spark-master -d phusitsom/spark-worker:3.3.2-hadoop3
...
`kubectl run spark-base --rm -it --labels="app=spark-client" --image phusitsom/spark-base:3.3.2-hadoop3 -- bash ./spark/bin/spark-shell --master spark://spark-master:7077 --conf spark.driver.host=spark-client`

To use `spark-submit` issue for example

`kubectl run spark-base --rm -it --labels="app=spark-client" --image phusitsom/spark-base:3.3.2-hadoop3 -- bash ./spark/bin/spark-submit --class CLASS_TO_RUN --master spark://spark-master:7077 --deploy-mode client --conf spark.driver.host=spark-client URL_TO_YOUR_APP`
